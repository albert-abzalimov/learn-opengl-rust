...
impl Shader {
    ...
    /// ------------------------------------------------------------------------
    pub unsafe fn setBool(&self, name: String, value: bool) {
        let cname = CString::new(name).unwrap();
        let loc = gl::GetUniformLocation(self.ID, cname.as_c_str().as_ptr());
        gl::Uniform1i(loc, value as i32);
    }
    /// ------------------------------------------------------------------------
    pub unsafe fn setInt(&self, name: String, value: i32) {
        let cname = CString::new(name).unwrap();
        let loc = gl::GetUniformLocation(self.ID, cname.as_c_str().as_ptr());
        gl::Uniform1i(loc, value);
    }
    /// ------------------------------------------------------------------------
    pub unsafe fn setFloat(&self, name: String, value: f32) {
        let cname = CString::new(name).unwrap();
        let loc = gl::GetUniformLocation(self.ID, cname.as_c_str().as_ptr());
        gl::Uniform1f(loc, value);
    }
    pub unsafe fn setMat4(&self, name: String, value: cgmath::Matrix4<f32>) {
        let cname = CString::new(name).unwrap();
        let loc = gl::GetUniformLocation(self.ID, cname.as_c_str().as_ptr());
